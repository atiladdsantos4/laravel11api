function listar_produtos(obj){//oficial//
      debugger;
      ...
      let config = {
        ...
        headers: { 
          ...
          'Authorization': document.getElementsByName('token')[0].getAttribute('content')
        },
        data : data
      };
...
$('.iativo').click(function(){
 ...
 function esconde(obj){
   ...
   //formata o json//
   let texto = $('#'+linha_ref).find("#requ"+id).val();
   teste = jQuery.parseJSON(texto);
   texto = JSON.stringify(teste, undefined, 4);
   $('#'+linha_ref).find("#requ"+id).val(texto);
...
function listar_produtos1(obj){//oficial//
  $("#spla").css({"visibility":"visible"});
  let path = $(obj).data("path");
  let path_dest = $(obj).data("path_dest");
  let id_request = $(obj).data("requ");
